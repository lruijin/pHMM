...
genSim <- function(N = 400, M = 2, K = c(2,2), ni = 5, hs = c(3,3,3),
                   tau=list(matrix(c(33.55,0.86,-3.99,3.81,-0.6,-0.09),3,2),matrix(c(39.015,-2.940,-3.075, 3.262,-0.014,-0.031),3,2)),
                   residinvvar=c(0.203, 64, 0.26, 52.6),
                   reSigma=matrix(c(12.978,0.043,9.645,-0.019,0.043,0.028,0.035,0.010,9.645,0.035,13.496,0.104,-0.019,0.010,0.104,0.029),nr=sum(K)),
                   P0=matrix(c(0.225,0.024,0.014,0.702,0.737,0.020,0.073,0.24,0.967),nr=3),Pi0=c(0.76,0.18, 0.06),
                   Pm0=list(matrix(c(0.163,0.005,0.005,0.632,0.973,0.101,0.204,0.022,0.894),3,3),matrix(c(0.6,0.031,0.013,0.381,0.918,0.063,0.019,0.051,0.924),3,3)),
                   P1=matrix(c(0.187,0.049,0.013,0.764, 0.682, 0.021, 0.049, 0.369, 0.966),nr=3),Pi1=c(0.76,0.18, 0.06),
                   Pm1=list(matrix(c(0.632,0.005,0.005,0.163,0.973,0.101,0.204,0.022,0.894),3,3),matrix(c(0.7,0.031,0.013,0.381,0.918,0.063,0.019,0.051,0.924),3,3)),
  ...
  for(i in 1:N) {
    ...
    for(m in 1:M){
      ...
      for(k in 1:K[m]){
        ...
        y[[ctt]][i,1]<- rnorm(1,tmpneta,sqrt(1/residinvvar[ctt]));
        ctt <- ctt + 1
      }
    }
    ...
    for(j in 2:ni) {
      ...
      for(m in 1: M){
        ...
        for(k in 1:K[m]){
          ...
          y[[ctt]][i,j] <- rnorm(1,tmpneta,sqrt(1/residinvvar[ctt]));
          ctt <- ctt + 1
        }
      }
  ...
  if(fitRx[1] & fitRx[2] & fitRx[3]) { #All Rx Effects
    mpars <- c(Pi0[1:(hs[1]-1)],as.numeric(t(P0))[-seq(hs[1]^2,1,-(hs[1]+1))], Pm0.out, unlist(tau), as.numeric(reSigma), residinvvar,as.numeric(t(P1))[-seq(hs[1]^2,1,-(hs[1]+1))],Pi1[1:(hs[1]-1)],Pm1.out);
  }
  if(!fitRx[2] & fitRx[1] & !fitRx[3]) { #Only Initial Prob
    mpars <- c(Pi0[1:(hs[1]-1)],as.numeric(t(P0))[-seq(hs[1]^2,1,-(hs[1]+1))], Pm0.out, unlist(tau), as.numeric(reSigma), residinvvar,Pi1[1:(hs[1]-1)]);
  }
  if(fitRx[2] & !fitRx[1] & !fitRx[3]) { #Only TPM Rx Effect
    mpars <- c(Pi0[1:(hs[1]-1)],as.numeric(t(P0))[-seq(hs[1]^2,1,-(hs[1]+1))], Pm0.out, unlist(tau), as.numeric(reSigma), residinvvar,as.numeric(t(P1))[-seq(hs[1]^2,1,-(hs[1]+1))]);
  }
  if(!fitRx[2] & !fitRx[1] & fitRx[3]) { #Only translation Rx Effect
    mpars <- c(Pi0[1:(hs[1]-1)],as.numeric(t(P0))[-seq(hs[1]^2,1,-(hs[1]+1))], Pm0.out, unlist(tau), as.numeric(reSigma), residinvvar,Pm1.out);
  }
  if(fitRx[2] & fitRx[1] & !fitRx[3]) { #Initial Prob and TPM Rx Effect
    mpars <- c(Pi0[1:(hs[1]-1)],as.numeric(t(P0))[-seq(hs[1]^2,1,-(hs[1]+1))], Pm0.out, unlist(tau), as.numeric(reSigma), residinvvar,as.numeric(t(P1))[-seq(hs[1]^2,1,-(hs[1]+1))],Pi1[1:(hs[1]-1)]);
  }
  if(fitRx[2] & !fitRx[1] & fitRx[3]) { #TPM and translation Rx Effect
    mpars <- c(Pi0[1:(hs[1]-1)],as.numeric(t(P0))[-seq(hs[1]^2,1,-(hs[1]+1))], Pm0.out, unlist(tau), as.numeric(reSigma), residinvvar,as.numeric(t(P1))[-seq(hs[1]^2,1,-(hs[1]+1))],Pm1.out);
  }
  if(!fitRx[2] & fitRx[1] & fitRx[3]) { #Initial Prob and translation Rx Effect
    mpars <- c(Pi0[1:(hs[1]-1)],as.numeric(t(P0))[-seq(hs[1]^2,1,-(hs[1]+1))], Pm0.out, unlist(tau), as.numeric(reSigma), residinvvar,Pi1[1:(hs[1]-1)],Pm1.out);
  }
  if(!fitRx[1] & !fitRx[2] & !fitRx[3])  { #No Rx Effect (Default)
    mpars <- c(Pi0[1:(hs[1]-1)],as.numeric(t(P0))[-seq(hs[1]^2,1,-(hs[1]+1))], Pm0.out, unlist(tau), as.numeric(reSigma), residinvvar);
  }
    ...
    for(int m=0; m<M;m++){
      ...
      for(int i = 0; i < hsm; i++){
        for(int k = 0; k< Km[m]; k++){
          probtmp[i] *= ::Rf_dnorm4(y(ctt+k,q),lambda[lmbs+i+k*(hsm+1)],lambda[lmbs + (k+1)*hsm + k],false);
        }
        probtmp[i] *= tmppm(r,i);
      }
      ctt = ctt + Km[m];
...
for(int q=0; q<n; q++){
  ...
  for(int m=0; m<M; m++){
    ...
    for(int k=0;k<Km[m];k++){
      for(int i=0;i<hsm;i++){
        probtmp[i] *= ::Rf_dnorm4(y(ctt,q),lambda[lmbs+i],lambda[lmbs + hsm],false);
      }
      lmbs += hsm + 1;
      ctt ++;
    }
...
for(int m=0; m<M; m++){
  ...
  NumericVector normb(states[m+1]);
  for(int k=0; k<Km[m];k++){
    for(int h=0; h<states[m+1]; h++){
    ...
    for(int h=0; h<states[m+1]; h++){
      lmb[ctt+h+ctsd] = tmpnb[h];
    }
    ctt += states[m+1];
    lmb[ctt+ctsd] = sdd[ctsd];
    ctsd ++;
  }
}
  ...
  for(int m=0; m<M; m++){
    for(int k=0; k<Km[m]; k++){
      for(int cf =0; cf<states[m+1]; cf++){
        lmbtmp[ctt+ctsd] = lmb[ctt+ctsd] + beta[repidx[ctsd*N + i]-1];
        ctt ++;
      }
      lmbtmp[ctt+ctsd] = lmb[ctt+ctsd];
      ctsd ++;
    }
  }
...
//List ret; ret["out"] = out; ret["lmbtmp"] = lmbtmp; ret["pm"] = pm; ret["tmpy"] = tmpy;ret["tmpZZ"]=tmpZZ;
List ret; ret["Z"] = Z;ret["Zm"] = Zm; ret["Zmax"] = Zmax; ret["Zmmax"] = Zmmax;
//List ret; ret["lmbmat"] = lmbmat;ret["probmat"] = probmat;
return ret;
';
...
simulated <- function(y,inits.,nsim.,report1.=1000,burnin = 3000,
  ...
  for(i in 1:nsim.) {
    ...
    ress <- t(matrix(o.betaa[reidx],nc=K)) #daf$randomEffects
    cvvv <- ress%*%t(ress);
    ...
    for(m in 1:M){
      ...
      for(k in 1:Km[m]){
        ...
        bbmean <- bbcov%*%t(ZZZ)%*%(y.na[ctt,]-n.betaa[reidx[(ctt-1)*N.+id.na]]);
        bbcov <- 1/o.betaa[invsigmaidx[ctt]]*bbcov;
        bbcov <- (bbcov + t(bbcov))/2
        n.betaa[tauidx[ctidx + (1:hs[m+1])]] <- rmvnorm(1,bbmean,bbcov);
        ctt <- ctt+1;
        ctidx <- ctidx+hs[m+1]
      }
    }
    ...
    for(m in 1:m){
      ZZZ <- ZZm[[m]]
      for(k in 1:Km[m]){
        n.betaa[invsigmaidx[ctt]] <- rgamma(1,(sum(ni.)/2 + a.param),
                                          b.param + sum((y.na[ctt,]-n.betaa[reidx[(ctt-1)*N.+id.na]] - ZZZ%*%n.betaa[tauidx[ctidx + (1:hs[m+1])]])^2)/2); #tau
        ctidx <- ctidx + hs[m+1]
        ctt <- ctt+1
      }
    }
