...
simulated <- function(y,X=NULL,inits.,nsim.,report1.=1000,burnin = 3000, no.random=F,
  ...
  if(!is.null(X)){
    ...
    pidx <- list(pi0=pi0idx,P0=P0idx,Pm0 = Pm0idx,tau=tauidx,beta = betaidx,Sigma=Sigmaidx)
  }else{
    Sigmaidx <- (max(tauidx)+1):(max(tauidx)+K*K);
    pidx <- list(pi0=pi0idx,P0=P0idx,Pm0 = Pm0idx,tau=tauidx,Sigma=Sigmaidx)
  }
  ...
  if(fitRx[1] & fitRx[2] & fitRx[3]) {
    ...
    reidx <- (max(Pm1idx)+1):(max(Pm1idx)+N.*K)
    pidx <- c(pidx,list(P1=Pm1idx,re=reidx))
    basepars <- max(pidx$P1)
  } else {
  ...
  for(i in 1:nsim.) {
    ...
    if(!is.null(X)){
      zz.ret <- pc.com(y.l,o.betaa,hs,Km,ni.,ccc,rx.,pidx,fitRx)
    }else{
    ...
    if(no.random){
      ...
      if(!is.null(X)){
        new.res <- re.n(y.l, Km, ZZm, o.betaa[1:basepars], n.betaa[1:basepars],ni., pidx, hs);
      }else{
        new.res <- re.n(y, Km, ZZm, o.betaa[1:basepars], n.betaa[1:basepars],ni., pidx, hs);
      }
      n.betaa[reidx] = as.vector(new.res)
    }
    #Simualte tau and beta
    ctt <- 1
    ctidx <- 0
    btidx <- 0
    for(m in 1:M){
      ...
      for(k in 1:Km[m]){
        ...
        if(!is.null(X)){
          bbmean <- bbcov %*% t(ZZZ) %*% invsigma %*% (y.l.na[ctt,]-n.betaa[reidx[(ctt-1)*N.+id.na]]);
          bbcov <- (bbcov + t(bbcov))/2
          n.betaa[tauidx[ctidx + (1:me)]] <- rmvnorm(1,bbmean,bbcov)
          bbcov <- solve(t(X) %*% invsigma %*% X)
          bbmean <- bbcov %*% t(X) %*% invsigma %*% (y.na[ctt,]-ZZZ %*% n.betaa[tauidx[ctidx+(1:me)]]-n.betaa[reidx[(ctt-1)*N.+id.na]]);
          bbcov <- (bbcov + t(bbcov))/2
          n.betaa[betaidx[btidx + (1:p)]] <- rmvnorm(1,bbmean,bbcov)
          btidx <- btidx + p
        }
        else{
    ...
    for(m in 1:M){
      ...
      for(k in 1:Km[m]){
        if(!is.null(X)){
          tau.k <- (y.na[ctt,]-n.betaa[reidx[(ctt-1)*N.+id.na]] - ZZZ%*%n.betaa[tauidx[ctidx + (1:me)]] - X %*%n.betaa[betaidx[btidx+(1:p)]])^2
          btidx <- btidx + p
        }else{
    ...
    if(i > burnin){
      if(!is.null(X)){
        llik_s = waic2(y.l, Km, ZZm,n.betaa,pidx,hs)
      }else{
