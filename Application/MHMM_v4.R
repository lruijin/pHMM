...
genSim_mhmm <- function(N = 390, K=4, ni=5, hs=3, tau=matrix(c(34.139,0.045,-3.150,3.341,-0.154,-0.028,37.676,-1.298,-3.168,3.288,-0.061,0.001),3,K),
                        residinvvar=matrix(c(0.066, 0.262, 0.256, 11.111, 106.281, 78.315, 0.142, 0.242, 0.216, 21.836, 132.118, 55.692),nr=hs,nc=K),
                        reSigma=matrix(c(13.105,0.048,9.548,-0.020,0.048,0.027,0.030,0.010,9.548,0.030,13.424,0.104,-0.020,0.010,0.104,0.029),nr=K),
                        P0=matrix(c(0.493,0.071,0.013,0.456,0.602,0.011,0.079,0.327,0.976),nr=3),Pi0=c(0.441,0.485,0.074),
                        P1=matrix(c(0.372,0.081,0.007,0.583,0.663,0.012,0.045,0.256,0.981),nr=3),Pi1=c(0.441,0.485,0.074),
  ...
  for(i in 1:N) {
    ...
    for(k in 1:K){
      ...
      y[[k]][i,1]<- rnorm(1,tmpneta,sqrt(1/residinvvar[Z[i,1],k]));
    }
    ...
    for(j in 2:ni) {
      ...
      for(k in 1:K){
        ...
        y[[k]][i,j] <- rnorm(1,tmpneta,sqrt(1/residinvvar[Z[i,j],k]));
      }
...
for(int q=0; q<n; q++) {
	for(int r=0; r<m; r++) {
	  double probtemp = ::Rf_dnorm4(y(0,q),lambda[r],lambda[m+r],false);
	  for(int k=1; k<K; k++){
	  probtemp = probtemp * ::Rf_dnorm4(y(k,q),lambda[k*m*2 + r], lambda[2*m*k + m + r],false);
	  }
...
NumericMatrix sdd(me,K); // use sd instead of inverse variance
for(int k=0;k<K;k++){
  for(int m=0;m<me;m++){
   sdd(m,k) = ::pow(beta[invsigidx[me*k+m]-1], -0.5);
  }
}
...
NumericVector lmb(K*me+K*me);
for(int k=0; k<K; k++) {
  for(int i=0; i<me; i++){
    lmb[k*(2*me) + i] = basenorm(i,k);
  }
  for(int i=0; i<me;i++){
    lmb[k*(2*me) + me + i] = sdd(i,k);
  }
}
...
for(int i = 0; i < N; i++){
  ...
  for(int k=0; k<K; k++) {
    for(int cf=0; cf<me; cf++){
      lmbtmp[cf+k*(me+me)] = basenorm(cf,k) + beta[repidx[k*N + i]-1];
    }
  }  
...
code_mhmm_re <- 'Environment mvtnorm("package:mvtnorm");
  ...
  // For common variance among classes
  ...
  // arma::mat newSigma = arma::inv(ninvdiag + invsig);
	...
	// For different variance among classes
  IntegerVector states(m);
  int me = states[0];
  NumericMatrix tmpsigma(me,K);
  IntegerVector errorSigidx = pidx["invsigma"];
  for(int k=0; k<K; k++){
    for(int i=0; i<me; i++){
      tmpsigma(i,k) = obeta[errorSigidx[k*me+i]-1];
    }
  }
	for(int i=me-1; i>0; i--){
	  tmpsigma.row(i) = tmpsigma.row(i) - tmpsigma.row(i-1);
	}
	arma::mat tmpsigman = Rcpp::as<arma::mat>(tmpsigma);
	
	arma::mat error(N*maxni,K);
	for(int k=0; k<K; k++){
	  error.col(k) = ZZa*tmpsigman.col(k);
	}
  ...
  for (int i=0; i<N; i++) {
    ...
    for(int j=1; j<maxni; j++){
      for(int k = 0; k< K;k++){
        ...
        tmpresid[k] = tmpy(i,j) - fixedn(i+j*N,k);
      }
    ...
    //  NumericMatrix tmpy = y[k];
    //  tmpSigma(k,k) = e
    //  tmpmean[k] = tmpy(i,0)- fixedn(i,k);
    //  for(int j=1; j<maxni; j++){
    //    tmpmean[k] += tmpy(i,j) - fixedn(j*N + i,k);
    //  }
    //}
	...
	return Res;
	//List ret; ret["newSigma"] = newSigma;ret["tmpmean"]=tmpmean; ret("tmpSigma") = tmpSigma;
	//ret["tmpresid"] = tmpresid; ret["fixedn"]=fixedn; ret["error"]=error; 
	//return ret;
';
...
code_waic <- '
  ...
  for(int k=0;k<K;k++){
    for(int m =0; m < me; m++){
      sdd(m,k) = ::pow(beta[invsigidx[k*me + m]-1], -0.5);
    }
  }
  for(int m = me-1; m>0; m--){
    sdd.row(m) = sdd.row(m) - sdd.row(m-1);
  }
  ...
  arma::mat tmpbetann = Rcpp::as<arma::mat>(tmpbetan);
  arma::mat tmpsdd = Rcpp::as<arma::mat>(sdd);
  
  arma::mat error(N*maxni, K);
  arma::mat fixedn(N*maxni,K);
  for(int k=0; k<K; k++){
    fixedn.col(k) = ZZa * tmpbetann.col(k); // obtain the fixed effect for each observation.
    error.col(k) = ZZa * tmpsdd.col(k); // obtain the measurement error for each observation.
  }
...
simulated_mhmm <- function(y,X = NULL,inits.,nsim.,report1.=1000,
  ...
  if(!is.null(X)){
    betaidx <- (max(tauidx) + 1) : (max(tauidx) + p * K);
    Sigmaidx <- (max(betaidx) + 1) : (max(betaidx) + K * K);
    pidx <- list(pi0=pi0idx,P0=P0idx,tau=tauidx,beta = betaidx,Sigma=Sigmaidx)
  }else{
    Sigmaidx <- (max(tauidx)+1):(max(tauidx)+K*K);
    pidx <- list(pi0=pi0idx,P0=P0idx,tau=tauidx,Sigma=Sigmaidx)
  }
  
  invsigmaidx <- (max(Sigmaidx)+1) : (max(Sigmaidx)+K*hs);
  pidx <- c(pidx,list(invsigma=invsigmaidx))
  
  if(fitRx[1] & fitRx[2]) {
    P1idx<- (max(invsigmaidx)+1):(max(invsigmaidx)+hs*(hs-1));
    pi1idx <- (max(P1idx)+1):(max(P1idx)+hs-1);
    reidx <- (max(pi1idx)+1):(max(pi1idx)+N.*K);
    pidx <- c(pidx,list(P1=P1idx,pi1=pi1idx,re=reidx))
  } else if(!fitRx[1] & fitRx[2]) {
    P1idx<- (max(invsigmaidx)+1):(max(invsigmaidx)+hs*(hs-1));
    reidx <- (max(P1idx)+1):(max(P1idx)+N.*K)
    pidx <- c(pidx,list(P1=P1idx,re=reidx))
  } else if(fitRx[1] & !fitRx[2]) {
    pi1idx <- (max(invsigmaidx)+1):(max(invsigmaidx)+hs-1);
    reidx <- (max(pi1idx)+1):(max(pi1idx)+N.*K);
    pidx <- c(pidx,list(pi1=pi1idx,re=reidx))
  } else {
    reidx <- (max(invsigmaidx)+1):(max(invsigmaidx)+N.*K);
    pidx <- c(pidx,list(re=reidx))
  }
  ...
  if(!is.null(X)){
    y.l.na <- matrix(NA,nr=K,nc=sum(inc))
    y.l <- y
    for(k in 1:K){
      y.l[[k]] <- y.l[[k]] - matrix(X %*% o.betaa[pidx$beta[(k-1)*p+(1:p)]],ncol=max(ni.),byrow=T)
      y.l.na[k,] <- y.l[[k]][inc]
    }
  }
  ...
  for(i in 1:nsim.) {
    ...
    if(!is.null(X)){
      zz.ret <- pc.com_mhmm(y.l,o.betaa,hs,ni.,ccc,rx.,pidx,fitRx)
    }else{
    ...
    if(no.random){n.betaa[reidx] = 0}else{
      if(!is.null(X)){
        new.res <- re.n_mhmm(y.l, ZZZ, o.betaa[1:basepars], n.betaa[1:basepars],ni., pidx, hs)
      }else{
    ...
    for(k in 1:K){
      if(emission.fix){
        ...
        n.betaa[betaidx[(k-1)*p + (1:p)]] <- o.betaa[betaidx[(k-1)*p + (1:p)]]
      }else{
        invsigma <- o.betaa[invsigmaidx[(k-1)*hs + (1:hs)]]
        invsigma <- c(invsigma[1],diff(invsigma))
        invsigma <- diag(c(ZZZ %*% matrix(invsigma,nc=1)))
        bbcov <- solve(t(ZZZ) %*% invsigma %*% ZZZ)
        if(!is.null(X)){
          bbmean <- bbcov%*%t(ZZZ) %*% invsigma %*%(y.l.na[k,]-n.betaa[reidx[(k-1)*N.+id.na]])
          bbcov <- (bbcov + t(bbcov))/2
          n.betaa[tauidx[(k-1)*hs + (1:hs)]] <- rmvnorm(1,bbmean,bbcov);
          bbcov <- solve(t(X) %*% invsigma %*% X)
          bbmean <- bbcov %*% t(X) %*% invsigma %*% (y.na[k,]-ZZZ %*% n.betaa[tauidx[(k-1) * hs+(1:hs)]]-n.betaa[reidx[(k-1)*N.+id.na]]);
          bbcov <- (bbcov + t(bbcov))/2
          n.betaa[betaidx[(k-1)*p + (1:p)]] <- rmvnorm(1,bbmean,bbcov)
        }else{
          bbmean <- bbcov%*%t(ZZZ) %*% invsigma %*%(y.na[k,]-n.betaa[reidx[(k-1)*N.+id.na]])
          bbcov <- (bbcov + t(bbcov))/2
          n.betaa[tauidx[(k-1)*hs + (1:hs)]] <- rmvnorm(1,bbmean,bbcov);
        }
      }
    ...
    if(emission.fix){
      for(k in 1:K){
        n.betaa[invsigmaidx[(k-1)*hs + (1:hs)]] <- o.betaa[invsigmaidx[(k-1)*hs + (1:hs)]]
      }
    }else{
      for(k in 1:K){
        if(!is.null(X)){
          tau.k <- (y.na[k,]-n.betaa[reidx[(k-1)*N.+id.na]] - ZZZ%*%n.betaa[tauidx[(k-1)*hs + (1:hs)]]- X %*%n.betaa[betaidx[(k-1)*p+(1:p)]])^2
        }else{
          tau.k <- (y.na[k,]-n.betaa[reidx[(k-1)*N.+id.na]] - ZZZ%*%n.betaa[tauidx[(k-1)*hs + (1:hs)]])^2
        }
        for(s in 1:hs){
          idx <- which(zz.na == s)
          rec <- length(idx)
          n.betaa[invsigmaidx[(k-1)*hs + s]] <- rgamma(1,(rec/2 + a.param),
                                                       b.param + sum(tau.k[idx])/2); #tau
        }
      }
    }
    ...
    if(!is.null(X)){
      ...
      y.l <- y
      for(k in 1:K){
        y.l[[k]] <- y.l[[k]] - matrix(X %*% n.betaa[pidx$beta[(k-1)*p+(1:p)]],ncol=max(ni.),byrow = T)
        y.l.na[k,] <- y.l[[k]][inc]
      }
    }
    ...
    if(i > burnin){
      if(!is.null(X)){
        llik_s = waic(y.l,ZZZ,n.betaa,pidx,hs)
      }else{
