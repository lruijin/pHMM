...
sim_each <- function(no.run,nsim.= 5000,burnin=3000){
  ...
  x_dhmm <- simulated(y = daf$y, inits. = c(daf$pars), nsim.= nsim., burnin = burnin, ksamp.= 1, Km = c(2,2), hs = c(3,3,3),
                      N.=daf$N, ni.=daf$ni, rx. = daf$rx, fitRx = daf$fitRx, report1.=1000,id=rep(1:daf$N,5),run. = no.run)
  inits.pc <- get_inits_pc(daf$pars,x_dhmm$pidx)
  ...
  x_mhmm_c <- simulated_mhmm(y = list(daf$y[[1]],daf$y[[2]]), inits.= get_inits_c(daf$pars,x_dhmm$pidx), nsim.= nsim., ksamp.= 1,
  ...
  x_mhmm_p <- simulated_mhmm(y = list(daf$y[[3]],daf$y[[4]]), inits.= get_inits_p(daf$pars,x_dhmm$pidx), nsim.= nsim., ksamp.= 1,
                             burnin = burnin,hs=3,N.=daf$N, K = 2, ni.=rep(5,daf$N), rx. = daf$rx, fitRx = daf$fitRx, report1.=1000,
                             id=rep(1:daf$N,5), hyperpar=c(3,1,1,.001,.0002), run. = no.run)
  ...
  x_dhmm2 <- simulated(y = daf$y, inits. = c(daf$pars,daf$pars[x_dhmm$pidx$Pm0]), nsim.= nsim., burnin = burnin, ksamp.= 1, 
  ...
  res <- list(confusion = confusion, true.param = daf$pars, 
              ...
              dhmm.cover <- (daf$pars > apply(x_dhmm$betaa[-(1:burnin),1:max(x_dhmm$pidx$P1)],2,quantile,0.025))&
                (daf$pars < apply(x_dhmm$betaa[-(1:burnin),1:max(x_dhmm$pidx$P1)],2,quantile,0.975)),
...
save('out',file="dhmm_model.Rdata")
stopCluster(cl)
mpi.exit()